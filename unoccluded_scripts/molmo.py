...
for entry in json_data:
    ...
    question = f"Count the exact number of {entry['dot_shape']}s in the image. Only count {entry['dot_shape']}s that are visible within the frame."
    print(question)
...
output_json_path = "unocc_results/molmo_syn_results.json"
...
for entry in json_data:
    ...
    question = f"Count the exact number of {entry['object']} in the image. Only count {entry['object']} that are visible within the frame. If {entry['object']} are partially in the frame (i.e. if any part of {entry['object']} are visible), count it."
    print(question)
    answer = get_answer(occluded_image_path, question, temperature=temperature)
    num = extract_num(answer)
...
# Save the updated JSON data back to the file
output_json_path = "unocc_results/molmo_real_results.json"
with open(output_json_path, 'w') as f:
    json.dump(json_data, f, indent=4)
