...
for entry in json_data:
    ...
    question = f"Count the exact number of {entry['dot_shape']}s in the image. Only count {entry['dot_shape']}s that are visible within the frame. If {entry['dot_shape']}s are partially in the frame (i.e. if any part of {entry['dot_shape']}s are visible), count it."
    print(question)
...
output_json_path = "unocc_results/intern_syn_results.json"
...
# Save the updated JSON data back to the file
output_json_path = "unocc_results/intern_real_results.json"
with open(output_json_path, 'w') as f:
    json.dump(json_data, f, indent=4)
